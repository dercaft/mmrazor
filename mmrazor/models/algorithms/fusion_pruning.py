...
# 对应的Script为: mmrazor/tests/simple_fusion/run_fusion_mmcls.sh
# 对应的Main Python为: mmrazor/tools/main_fusion_pruning.py
    # mmrazor/tests/simple_fusion/test_mmcls.py
# 对应的Algorithm为: mmrazor/models/algorithms/fusion_pruning.py
# 对应的Pruner为: mmrazor/models/pruners/layer_pruning.py
@ALGORITHMS.register_module()
class FusionPruningAlgoritm(BaseAlgorithm):
    ...
    def get_subnet_flops(self):
        ...
        for name, module in self.architecture.named_modules():
            ...
            # if name in self.pruner.subnet_dict or sn in self.pruner.subnet_dict: # 若在被删除层字典里 self.pruned_layers_name
            if name in self.pruned_layers_name or sn in self.pruned_layers_name: # 若在被删除层字典里 self.pruned_layers_name
                continue
    ...
    def fusion_pipeline(self,**kwargs):
        ...
        space2name,features_dict, features_pool = extract_features(self,dataloader,get_pool=True)
        if features_pool:
    ...
    def compare_features(self,dataloader,aim_layer,pruned_layer):
        ...
        space2name,features_dict,_=extract_features(self,dataloader,get_pool=False)
