...
from covariance import cov_norm, cov_eig, cov_eig_kl, cov_kl
from kernel import alignment
...
if __name__ == "__main__":
    import time
    x, y = torch.randn((2048, 2048)), torch.randn((2048, 2048))
    met=get_sim_measure('emd')
    start=time.time()
    sim=compute_similarity(x, y, met,'emd', samp_size=1024)
    cost=time.time()-start
    print("SIMILARITY: {} COST: {}".format(sim, cost))