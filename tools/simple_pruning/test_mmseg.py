...
from mmseg.apis import multi_gpu_test, single_gpu_test, get_root_logger
...
def main():
    ...
    if rank==0:
        ...
        algorithm.compress(0.5)
    else:
        logger.info(f"RANK {rank} NOT compress")
    rflop=algorithm.get_raw_flops()
    ...
    #
    logger.info(f"RANK {rank} REDO compress")
    algorithm.compress(0.5)
    rflop=algorithm.get_raw_flops()
    sflop=algorithm.get_subnet_flops()
    logger.info(f"RANK {rank} REDO Raw model flop is: {rflop}")
    logger.info(f"RANK {rank} REDO Sub model flop is: {sflop}")
    # 
