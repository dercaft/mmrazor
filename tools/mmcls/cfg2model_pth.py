...
def parse_args():
    ...
    # Adder
    parser.add_argument(
        '--model_file_path', type=str,default='', help='filepath containing \
        channels cfg of subnets. Only applicable to non-distributed training.')
    parser.add_argument(
        '--model_index', type=int, default=-1, help='index of subnet model. \
...
def main():
    ...
    if space2ratio:
        ...
        # 不知道用 algorithm.architecture 还是 algorithm自己
        arch=algorithm.comprss_deploy(space2ratio)
    else:
    ...
    arch=arch.cpu()
    torch.save(algorithm, osp.join(cfg.work_dir, 'algorithm.pth'))
    # mmcv.save(algorithm, osp.join(cfg.work_dir, 'algorithm.pth'))
    # mmcv.save(arch, osp.join(cfg.work_dir, 'architecture.pth'))
    torch.save(arch, osp.join(cfg.work_dir, 'architecture.pth'))
